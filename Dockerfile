FROM ubuntu:20.04 as TWCoreBuilder
...
# RUN echo "GOPATH=/usr/local/go" >> ~/.bashrc \ 
#     && echo "export GOPATH" >> ~/.bashrc \ 
#     && echo "PATH=$PATH:/usr/local/go/bin # Add GOPATH/bin to PATH for scripting" >> ~/.bashrc 
    
ENV PATH="/usr/local/go/bin:${PATH}"
# ENV GOPATH="/usr/local/go"
# RUN /usr/local/go/bin/go version 
# CMD [ "/bin/bash","-c","source ~/.bashrc" ]
# CMD [ "/bin/bash","-c","echo DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD" ]
...
# COPY . . 
COPY --from=TWCoreBuilder /wallet-core ./bin/wallet-core
RUN make  
